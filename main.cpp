...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	/*GetCPUDescriptorHandle(DescriptorHeap, descriptorSizeRTV, 0);*/
#ifdef _DEBUG
	...
	while (msg.message != WM_QUIT) {
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			*wvpData = worldMatrix;
			...
			ImGui::Begin("Window");
			float setColor[] = {RGB[0], RGB[1], RGB[2]};
			ImGui::SliderFloat4("Color", setColor, 0.0f, 1.0f);// 色
			RGB[0] = setColor[0];
			RGB[1] = setColor[1];
			RGB[2] = setColor[2];

			ImGui::ColorEdit4("Color", setColor);

			ImGui::End();

			ImGui::Render();

			*materialData = Vector4(RGB[0], RGB[1], RGB[2], 1.0f);
